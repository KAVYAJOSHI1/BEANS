...
| | |
|---|---|
| Fused risk PR-AUC (random = 0.066) | **0.950**; without network-layer features 0.917 |
| Alerts that are illicit / illicit entities alerted | **98 %** / **97 %** (29 of 30) |
| Typology shown on alerts that is correct | 84 % |
| E3 transaction-shape classifier macro-F1 | 0.993 |
| Calibration error (ECE) | 0.005 |
| End-to-end run (ingest + 4 engines + training) | 15 s on a laptop; scoring ≈ 3,300 rows/s ([benchmark](docs/BENCHMARK.md)) |

**Across 6 independently generated datasets** (`scripts/evaluate_seeds.py --seeds 42 7 123 2024 99 555`), mean ± std:
PR-AUC 0.953 ± 0.013, recall at P ≥ 0.5 0.891 ± 0.033, precision at P ≥ 0.5 0.964 ± 0.015, alert precision 0.967 ± 0.028,
alert typology accuracy 0.857 ± 0.053. Use this benchmark to judge any model change, not a single dataset.
